...
async function loadCryptos() {
    ...
    try {
        ...
        // Calcular MA50 para cada cripto (top 100)
        cryptoMA50Data = [];
        const top100 = cryptoData.slice(0, 100);
        
        for (let i = 0; i < top100.length; i++) {
            const crypto = top100[i];
            document.getElementById('loadingText').textContent = 
                `Calculando MA50: ${crypto.symbol} (${i + 1}/${top100.length})`;
...
// Calcular MA50 para uma cripto (usa API com histórico COMPLETO desde 2017)
async function calculateMA50(symbol) {
    // Usar o endpoint de backtest que busca histórico completo
    const response = await fetch(`/api/ma50-backtest/${symbol}`);
    const data = await response.json();
    
    if (data.error || data.days < 50) {
        return { ma50: null, signal: 'N/A', distance: 0, ma50Profit: 0, holdProfit: 0 };
    }
    ...
    return { 
        ...
        holdProfit: data.holdProfit,
        ma50Profit: data.ma50Profit,
        trades: data.trades,
        startDate: data.startDate,
        daysTrading: data.days,
        winner: data.winner
    };
}
...
function displayCryptos() {
    ...
    tbody.innerHTML = filteredData.map((crypto, index) => {
        ...
        // Classes para lucros
        const ma50ProfitClass = crypto.ma50Profit >= 0 ? 'price-up' : 'price-down';
        const holdProfitClass = crypto.holdProfit >= 0 ? 'price-up' : 'price-down';
        const winnerBadge = crypto.winner === 'MA50' 
            ? '<span class="badge bg-success ms-1">🏆</span>' 
            : '<span class="badge bg-secondary ms-1">HOLD</span>';
        
        // Formatar valor da MA50
        const ma50Value = crypto.ma50 ? `$${formatNumber(crypto.ma50)}` : '-';
        ...
        return `
            <tr>
                ...
                <td class="text-info" title="Média dos últimos 50 dias">${ma50Value}</td>
                <td class="${ma50ProfitClass}" title="Lucro usando estratégia MA50 (${crypto.trades || 0} trades)">
                    <strong>${crypto.ma50Profit >= 0 ? '+' : ''}${(crypto.ma50Profit || 0).toFixed(0)}%</strong>
                    ${winnerBadge}
                </td>
                <td class="${holdProfitClass}" title="Lucro apenas segurando (HOLD)">
                    ${crypto.holdProfit >= 0 ? '+' : ''}${(crypto.holdProfit || 0).toFixed(0)}%
                </td>
                <td class="${maClass}">
                    ${crypto.distance >= 0 ? '+' : ''}${crypto.distance.toFixed(1)}%
                </td>
                <td class="${priceClass}">
                    ${crypto.priceChangePercent >= 0 ? '+' : ''}${crypto.priceChangePercent.toFixed(1)}%
                </td>
                <td>${signalBadge}</td>
                <td>
                    <button class="btn btn-sm btn-outline-primary" onclick="showChart('${crypto.symbol}')" title="Ver gráfico">
